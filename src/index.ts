...
async function loadStorefrontScript(
  groupID: number,
  styling?: StylingParameter,
  ...
  try {
    ...
    // Don't use shared styling for Canes, TGK
    if (![58, 111].includes(groupID)) {
    ...
    const uniqueScript = await import(
      /* webpackChunkName: "uniqueScript" */ `./store_scripts/${scriptFolder}/index.ts`
    );
    await import(
      /* webpackChunkName: "uniqueStyling" */ `./store_scripts/${scriptFolder}/styles.css`
    );
...
function setCSSVariables(stylingVariables: StylingParameter) {
  ...
  for (const variableName in stylingVariables) {
    const defaultCSSValue = root.css(
      StylingParameterMetadata[variableName as keyof StylingParameter]
        .CSSVariableName
    );
    if (variableName in StylingParameterMetadata) {
      root.css(
        StylingParameterMetadata[variableName as keyof StylingParameter]
          .CSSVariableName,
        stylingVariables[variableName as keyof StylingParameter] ??
          defaultCSSValue
      );
    }
  }
